...
% makima Interpolated matrix
lvim = interp1(listed_expiry_dates,lv_matrix,simulation_dates,'makima');
...
% Building Local Volatility Surface : makima
figure
surf(A,C,h)
title('makima Interpolation of Local Volatility','FontSize',30)
...
% Interpolating Forward Prices for 28 days
lv_4 = interp1(listed_expiry_dates,Imp,simulation_dates,'makima');
ImpFwd_makima = transpose(lv_4);


% MONTE CARLO SIMULATION - MAKIMA
St_price = 3076.74;
dt=1/252;
N = 10000;
e = randn(28,N);
...
forwardPrice_makima = ImpFwd_makima;
LocalVolMoneyness = B;
LocalVolSurface_makima = lvim;
for i=1:1:27
lv_makima = interp1(LocalVolMoneyness,LocalVolSurface_makima(i+1,:),simulations(i,:)./forwardPrice_makima(i),'makima','extrap');
simulations(i+1,:) = simulations(i,:).*exp((mu-.5*lv_makima.^2)*dt+lv_makima.*sqrt(dt).*e(i,:));
end

USt_makima = [St_price zeros(1,28-1)];
LSt_makima = [St_price zeros(1,28-1)];
ESt_makima = [St_price zeros(1,28-1)];
lv2_makima(1,1)=interp1(LocalVolMoneyness,LocalVolSurface_makima(1,:),ESt_makima(1,1)./forwardPrice_makima(1),'makima','extrap');

%95 percent confidence interval%

for i=1:1:27
    ESt_makima(i+1)=simulations(1,1)*exp(mu*i*dt);
    USt_makima(i+1)=ESt_makima(i+1)+1.96*lv2_makima(1,i)*simulations(1,1)*sqrt(i*dt);
    LSt_makima(i+1)=ESt_makima(i+1)-1.96*lv2_makima(1,i)*simulations(1,1)*sqrt(i*dt);
    lv2_makima(1,i+1)=interp1(LocalVolMoneyness,LocalVolSurface_makima(i+1,:),ESt_makima(1,i+1)./forwardPrice_makima(i+1),'makima','extrap');
end
...
xlabel('T (days)');
hold on;
plot(ESt_makima,'--','color','black','LineWidth',6);
plot(USt_makima,'--','color','yellow','LineWidth',3);
plot(LSt_makima,'--','color','yellow','LineWidth',3);
...
CBOEsigma = interp1([125,130],[0.0163,0.0192],CurrentSkewIndex,'makima');
ModelSigma = ksdensity(trans_ret,bl,'function','cdf');


skewint = interp1([0.0074,0.0104],[110,115],ModelSigma,'makima');