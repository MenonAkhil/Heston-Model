...
% pchip interpolated matrix
imlv = interp1(listed_expiry_dates,lv_matrix,simulation_dates,'pchip');
...
% Building Local Volatility Surface : pchip
figure
surf(A,C,g)
title('pchip Interpolation of Local Volatility','FontSize',30)
...
lv_3 = interp1(listed_expiry_dates,Imp,simulation_dates,'pchip');
ImpFwd_pchip = transpose(lv_3);



% MONTE CARLO SIMULATION - PCHIP
St_price = 3076.74;
dt=1/252;
N = 10000;
e = randn(28,N);
...
forwardPrice_pchip = ImpFwd_pchip;
LocalVolMoneyness = B;
LocalVolSurface_pchip = imlv;
for i=1:1:27
lv_pchip = interp1(LocalVolMoneyness,LocalVolSurface_pchip(i+1,:),simulations(i,:)./forwardPrice_pchip(i),'pchip','extrap');
simulations(i+1,:) = simulations(i,:).*exp((mu-.5*lv_pchip.^2)*dt+lv_pchip.*sqrt(dt).*e(i,:));
end

USt_pchip = [St_price zeros(1,28-1)];
LSt_pchip = [St_price zeros(1,28-1)];
ESt_pchip = [St_price zeros(1,28-1)];
lv2_pchip(1,1)=interp1(LocalVolMoneyness,LocalVolSurface_pchip(1,:),ESt_pchip(1,1)./forwardPrice_pchip(1),'pchip','extrap');

%95 percent confidence interval%

for i=1:1:27
    ESt_pchip(i+1)=simulations(1,1)*exp(mu*i*dt);
    USt_pchip(i+1)=ESt_pchip(i+1)+1.96*lv2_pchip(1,i)*simulations(1,1)*sqrt(i*dt);
    LSt_pchip(i+1)=ESt_pchip(i+1)-1.96*lv2_pchip(1,i)*simulations(1,1)*sqrt(i*dt);
    lv2_pchip(1,i+1)=interp1(LocalVolMoneyness,LocalVolSurface_pchip(i+1,:),ESt_pchip(1,i+1)./forwardPrice_pchip(i+1),'pchip','extrap');
end
...
xlabel('T (days)');
hold on;
plot(ESt_pchip,'--','color','black','LineWidth',6);
plot(USt_pchip,'--','color','yellow','LineWidth',3);
plot(LSt_pchip,'--','color','yellow','LineWidth',3);
...
CBOEsigma = interp1([125,130],[0.0163,0.0192],CurrentSkewIndex,'pchip');
ModelSigma = ksdensity(trans_ret,bl,'function','cdf');


skewint = interp1([0.0074,0.0104],[110,115],ModelSigma,'pchip');