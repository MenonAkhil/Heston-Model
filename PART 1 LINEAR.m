% FI6121 - 40% ASSIGNMENT - AKHIL MENON
...
% Linear Interpolated Matrix (Local Volatility Surface Matrix)
interpolated_lvmatrix=interp1(listed_expiry_dates,lv_matrix,simulation_dates,'linear');
...
C = B';
F = transpose(interpolated_lvmatrix);


% Building Local Volatility Surface : Linear
figure
surf(A,C,F)
title('Linear Interpolation of Local Volatility : Coarse Mesh','FontSize',30)
...
lv_2 = interp1(listed_expiry_dates,Imp,simulation_dates,'linear');
ImpFwd_linear = transpose(lv_2);
...
forwardPrice_linear = ImpFwd_linear;
LocalVolMoneyness = B;
LocalVolSurface_linear = interpolated_lvmatrix;
for i=1:1:27
lv_linear = interp1(LocalVolMoneyness,LocalVolSurface_linear(i+1,:),simulations(i,:)./forwardPrice_linear(i),'linear','extrap');
simulations(i+1,:) = simulations(i,:).*exp((mu-.5*lv_linear.^2)*dt+lv_linear.*sqrt(dt).*e(i,:));
end

USt_linear = [St_price zeros(1,28-1)];
LSt_linear = [St_price zeros(1,28-1)];
ESt_linear = [St_price zeros(1,28-1)];
lv2_linear(1,1)=interp1(LocalVolMoneyness,LocalVolSurface_linear(1,:),ESt_linear(1,1)./forwardPrice_linear(1),'linear','extrap');

%95 percent confidence interval%

for i=1:1:27
    ESt_linear(i+1)=simulations(1,1)*exp(mu*i*dt);
    USt_linear(i+1)=ESt_linear(i+1)+1.96*lv2_linear(1,i)*simulations(1,1)*sqrt(i*dt);
    LSt_linear(i+1)=ESt_linear(i+1)-1.96*lv2_linear(1,i)*simulations(1,1)*sqrt(i*dt);
    lv2_linear(1,i+1)=interp1(LocalVolMoneyness,LocalVolSurface_linear(i+1,:),ESt_linear(1,i+1)./forwardPrice_linear(i+1),'linear','extrap');
end
...
xlabel('T (days)');
hold on;
plot(ESt_linear,'--','color','black','LineWidth',6);
plot(USt_linear,'--','color','yellow','LineWidth',3);
plot(LSt_linear,'--','color','yellow','LineWidth',3);
...
CBOEsigma = interp1([125,130],[0.0163,0.0192],CurrentSkewIndex,'linear');
ModelSigma = ksdensity(trans_ret,bl,'function','cdf');


skewint = interp1([0.0074,0.0104],[110,115],ModelSigma,'linear');